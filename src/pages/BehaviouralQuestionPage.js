...
function BehaviouralQuestionPage() {
  ...
  useEffect(() => {
    import("../data/behavioural.json").then((mod) => {
      const loadedTopics = (mod.behaviouralTopics || []).map((topic) => ({
        ...topic,
      ...
      if (!foundQuestion) navigate("/behavioural");
    });
...
export default BehaviouralQuestionPage;
